...
// ─── Task Modal ─────────────────────────────────────────────
function TaskModal({ mode, initial, projects, users, departments, onClose, onSave, saving }) {
  const [form, setForm] = useState(
    initial
      ? {
          ...
          reviewers: (initial.reviewers || []).map(r => r.userId?._id || r.userId || r),
  ...
  const filteredUsers = (users || []).filter((u) => {
    ...
    const deptMatch = !filterDept || String(u.departmentId?._id || u.departmentId) === filterDept;
    const roleMatch = !filterRole || u.globalRole === filterRole;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl bg-surface-low border border-outline shadow-2xl">
        ...
        <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[80vh] overflow-y-auto scrollbar-thin">
          ...
          <div className="space-y-4">
            <Field label="Assign Contributors & Reviewers">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                    <select 
                        ...
                        className="flex-1 bg-surface-container border border-outline px-2 py-1 text-[10px] text-foreground focus:outline-none"
                    >
                ...
                <div className="bg-surface-container border border-outline h-64 overflow-y-auto scrollbar-thin divide-y divide-outline">
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((u) => {
                      const id = u._id;
                      const isContr = form.contributors.includes(id);
                      ...
                      return (
                        <div key={id} className="p-2 flex items-center justify-between">
                          ...
                          <div className="flex gap-1">
                            <button
                              type="button"
                              onClick={() => toggleUser(id, 'contributors')}
                              className={`px-2 py-1 text-[9px] font-bold uppercase border transition-colors ${isContr ? 'bg-primary border-primary text-on-primary' : 'border-outline text-foreground-muted hover:border-foreground'}`}
                            >
                              Contr.
                            </button>
                            <button
                              type="button"
                              onClick={() => toggleUser(id, 'reviewers')}
                              className={`px-2 py-1 text-[9px] font-bold uppercase border transition-colors ${isRev ? 'bg-[#e8a847] border-[#e8a847] text-white' : 'border-outline text-foreground-muted hover:border-foreground'}`}
                            >
                              Rev.
                            </button>
                          </div>
        ...
        <div className="flex gap-2 px-6 pb-5 pt-2">
          <button onClick={onClose} disabled={saving} className="flex-1 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold border border-outline text-foreground-muted hover:text-foreground transition-colors disabled:opacity-40">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={saving} className="flex-1 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold bg-primary text-on-primary hover:bg-primary/90 transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-sm bg-surface-low border border-outline shadow-2xl p-6">
        ...
        <div className="flex gap-2">
          <button onClick={onClose} disabled={saving} className="flex-1 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold border border-outline text-foreground-muted hover:text-foreground transition-colors">Cancel</button>
          <button onClick={onConfirm} disabled={saving} className="flex-1 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold bg-[#ff4747] text-foreground hover:bg-[#e03d3d] transition-colors flex items-center justify-center gap-2">
...
export default function DeptHeadTasks() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [allTasks, allProjects, allUsers, allDepts] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-y-3">
        ...
        <button onClick={() => setModal({ type: 'add' })} className="flex items-center gap-2 px-4 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold bg-primary text-on-primary hover:bg-primary/90 transition-colors">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tasks…" className="w-full bg-surface-container border border-outline pl-9 pr-3 py-2.5 text-[12px] text-foreground focus:outline-none focus:border-primary transition-colors" />
        </div>
        <div className="relative">
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="appearance-none bg-surface-container border border-outline px-3 py-2.5 pr-8 text-[12px] text-foreground focus:outline-none focus:border-primary cursor-pointer">
        ...
        <div className="border border-outline bg-surface-low overflow-hidden min-w-145">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-outline">
              {filtered.map((t) => (
                <div key={t._id} className="grid grid-cols-[2fr_1fr_1fr_120px_60px] gap-4 px-6 py-4 items-center hover:bg-surface-container transition-colors group">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setModal({ type: 'edit', task: t })} className="p-1.5 text-foreground-muted hover:text-foreground transition-colors"><Pencil className="w-3 h-3" /></button>
                    <button onClick={() => setModal({ type: 'delete', task: t })} className="p-1.5 text-foreground-muted hover:text-[#ff4747] transition-colors"><Trash2 className="w-3 h-3" /></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {modal?.type === 'add' && <TaskModal mode="add" projects={projects} users={users} departments={departments} onClose={() => setModal(null)} onSave={handleAdd} saving={saving} />}
      {modal?.type === 'edit' && <TaskModal mode="edit" initial={modal.task} projects={projects} users={users} departments={departments} onClose={() => setModal(null)} onSave={handleEdit} saving={saving} />}
      {modal?.type === 'delete' && <DeleteModal task={modal.task} onClose={() => setModal(null)} onConfirm={handleDelete} saving={saving} />}
    </div>
  );
}
