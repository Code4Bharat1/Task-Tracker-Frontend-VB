...
import {
  BookCheck, Search, ChevronDown, AlertCircle, User, FolderKanban, Calendar, X,
} from "lucide-react";
import { TableSkeleton, FilterSkeleton, HeaderSkeleton } from "@/components/skeletons";
...
function parseDate(raw) {
  ...
  if (typeof raw === "object") {
    ...
    if (raw.toISOString) { try { return new Date(raw.toISOString()); } catch {} }
    try { raw = JSON.stringify(raw); } catch {}
  }
...
function LogDetailModal({ log, onClose }) {
  ...
  const entries = log.entries?.length
    ? log.entries
    : [{ projectName: log.projectName, moduleTitle: log.moduleTitle, description: log.description }];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-surface-low border border-outline shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-outline shrink-0">
          <span className="text-[11px] tracking-[0.2em] uppercase font-bold text-foreground">Work Log Detail</span>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto">
          <div className="flex items-center gap-3 pb-3 border-b border-outline">
            ...
            <div>
              <p className="text-[13px] font-bold text-foreground">{log.userName}</p>
              <p className="text-[10px] text-foreground-muted tracking-[0.1em] uppercase">{formatDateTime(log.created_at)}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-foreground-muted">
            <Calendar className="w-3.5 h-3.5" />
            <span className="text-[11px] tracking-[0.1em]">Log Date: {formatDate(log.logDate)}</span>
          </div>
          <div className="space-y-3">
            ...
            {entries.map((entry, i) => (
              <div key={i} className="bg-surface-container border border-outline p-4 space-y-2">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[10px] tracking-[0.12em] uppercase text-foreground-muted mb-0.5">Project</p>
                    <p className="text-[12px] text-foreground font-medium">{entry.projectName || "—"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] tracking-[0.12em] uppercase text-foreground-muted mb-0.5">Module</p>
                    <p className="text-[12px] text-foreground font-medium">{entry.moduleTitle || "N/A"}</p>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] tracking-[0.12em] uppercase text-foreground-muted mb-0.5">Description</p>
                  <p className="text-[12px] text-foreground-muted leading-relaxed">{entry.description || "—"}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="px-6 pb-5 shrink-0">
          <button onClick={onClose} className="w-full py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold border border-outline text-foreground-muted hover:text-foreground hover:border-foreground-muted transition-colors">
            Close
          </button>
        </div>
      </div>
...
export default function DailyLogsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [logsR, projectsR] = await Promise.allSettled([getAllLogs(), getProjects()]);
      let normalizedLogs = [];
  ...
  const filtered = logsArr.filter((l) => {
    const entries = l.entries?.length
      ? l.entries
      : [{ projectName: l.projectName, moduleTitle: l.moduleTitle, description: l.description }];
    ...
    const matchProject = filterProject === "all" ||
      entries.some((e) => String(e.projectId?._id || e.projectId) === String(filterProject));
    const matchDate = !filterDate || (() => {
      const d = l.logDate ?? l.date ?? l.created_at;
      if (!d) return false;
      try { return new Date(d).toISOString().startsWith(filterDate); }
      catch { return String(d).startsWith(filterDate); }
    })();
    return matchSearch && matchProject && matchDate;
  });

  const totalEntries = filtered.length;
  const uniqueContributors = new Set(filtered.map((l) => String(l.userId))).size;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-y-3">
        <div>
          <p className="text-[10px] tracking-[0.2em] uppercase text-foreground-muted mb-1">Overview</p>
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Daily Work Logs</h1>
        </div>
        <div className="flex gap-4">
          <div className="border border-outline bg-surface-container px-4 py-2.5 text-center">
            <p className="text-[10px] tracking-[0.15em] uppercase text-foreground-muted">Total Logs</p>
            <p className="text-xl font-bold text-foreground mt-0.5">{totalEntries}</p>
          </div>
          <div className="border border-outline bg-surface-container px-4 py-2.5 text-center">
            <p className="text-[10px] tracking-[0.15em] uppercase text-foreground-muted">Contributors</p>
            <p className="text-xl font-bold text-primary mt-0.5">{uniqueContributors}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Search by employee, project, module…"
        ...
        {(filterDate || filterProject !== "all" || search) && (
          <button
            onClick={() => { setSearch(""); setFilterDate(""); setFilterProject("all"); }}
      ...
      <div className="overflow-x-auto">
        <div className="border border-outline bg-surface-low overflow-hidden min-w-150">
          <div className="grid grid-cols-[1fr_1fr_1fr_120px_100px] gap-4 px-6 py-3 border-b border-outline bg-surface-container items-center">
            {["Employee", "Project", "Module", "Log Date", ""].map((h) => (
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-foreground-muted">
              ...
              <p className="text-[12px] tracking-[0.1em] uppercase">No logs found</p>
            </div>
          ) : (
            <div className="divide-y divide-outline">
              {filtered.map((log) => {
                const firstEntry = log.entries?.[0] || { projectName: log.projectName, moduleTitle: log.moduleTitle };
                return (
                  <button
                    key={log._id}
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <span className="text-[12px] font-medium text-foreground truncate">{log.userName}</span>
                    </div>
                    <div className="flex items-center gap-1.5 min-w-0">
                      <FolderKanban className="w-3 h-3 text-foreground-muted shrink-0" />
                      <span className="text-[12px] text-foreground-muted truncate">{firstEntry.projectName || "—"}</span>
                    </div>
                    <div className="flex items-center min-w-0">
                      <span className="text-[12px] text-foreground-muted truncate">{firstEntry.moduleTitle || "—"}</span>
                    </div>
                    <span className="text-[12px] text-foreground-muted">{formatDate(log.logDate)}</span>
                    <span className="text-[11px] text-foreground-muted group-hover:text-primary transition-colors">View →</span>
                  </button>
                );
              })}
            </div>
