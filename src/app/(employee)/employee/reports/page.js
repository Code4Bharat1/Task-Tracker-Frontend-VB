            ...
            {filtered.map((opt) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onChange(
                      selected ? value.filter((k) => k !== opt.key) : [...value, opt.key],
                    )
                  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-surface-low border border-outline shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] tracking-[0.15em] uppercase text-foreground-muted mb-1.5">Weekly Report</label>
              <div className="flex gap-2">
                {[true, false].map((v) => (
                  <button
                    key={String(v)}
                    type="button"
                    onClick={() => setForm({ ...form, weeklyIncluded: v })}
                    className={`flex-1 py-2 text-[11px] font-bold uppercase border transition-colors ${form.weeklyIncluded === v ? "bg-primary border-primary text-on-primary" : "border-outline text-foreground-muted"}`}
                  >
                    {v ? "Yes" : "No"}
                  </button>
                ))}
            ...
            <div>
              <label className="block text-[10px] tracking-[0.15em] uppercase text-foreground-muted mb-1.5">Monthly Report</label>
              <div className="flex gap-2">
                {[true, false].map((v) => (
                  <button
                    key={String(v)}
                    type="button"
                    onClick={() => setForm({ ...form, monthlyIncluded: v })}
                    className={`flex-1 py-2 text-[11px] font-bold uppercase border transition-colors ${form.monthlyIncluded === v ? "bg-primary border-primary text-on-primary" : "border-outline text-foreground-muted"}`}
                  >
                    {v ? "Yes" : "No"}
                  </button>
                ))}
        ...
        <div className="flex gap-2 px-6 pb-5 pt-2">
          <button onClick={onClose} disabled={saving} className="flex-1 py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold border border-outline text-foreground-muted hover:text-foreground disabled:opacity-40">
            Cancel
          </button>
...
function ReportsPageInner() {
  ...
  const [modal, setModal] = useState(null); // null | { type: "add" } | { type: "edit", report }
  ...
  const [filterMonthly, setFilterMonthly] = useState("all");
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (Array.isArray(remote)) {
        setReports(remote.map(normalize));
      }
    } catch {
  ...
  async function toggleField(id, field) {
    ...
    try {
      await updateReport(id, { [field]: updated[field] });
    } catch {
      // revert on failure
  ...
  const filtered = reports.filter((r) => {
    if (search && !r.notes?.toLowerCase().includes(search.toLowerCase()) &&
        !r.clientResponse?.toLowerCase().includes(search.toLowerCase()) &&
    ...
    if (filterWeekly !== "all" && String(r.weeklyIncluded) !== filterWeekly) return false;
    if (filterMonthly !== "all" && String(r.monthlyIncluded) !== filterMonthly) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-3">
        <div>
          <p className="text-[10px] tracking-[0.2em] uppercase text-foreground-muted mb-1">Activity</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search notes, response, project…"
        ...
        <select
          ...
          className="bg-surface-container border border-outline px-3 py-2.5 text-[12px] text-foreground focus:outline-none focus:border-primary"
        >
          <option value="all">Weekly: All</option>
          <option value="true">Weekly: Yes</option>
          <option value="false">Weekly: No</option>
        </select>
        <select
          value={filterMonthly}
        ...
        >
          ...
          <option value="true">Monthly: Yes</option>
          <option value="false">Monthly: No</option>
        </select>
      </div>

      {/* Table */}
      <div className="overflow-x-auto">
        <div className="border border-outline bg-surface-low min-w-[900px]">
          {/* Header */}
          <div className="grid grid-cols-[120px_160px_1fr_1fr_130px_130px_60px] gap-0 border-b border-outline bg-surface-container">
            {["Date", "Update Type", "Update", "Client Response", "Weekly Report", "Monthly Report", ""].map((h) => (
              <div key={h} className="px-4 py-3 text-[10px] tracking-[0.15em] uppercase text-foreground-muted font-bold border-r border-outline last:border-r-0">
                {h}
              </div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-outline">
              {filtered.map((r) => (
                <div
                  key={r.id}
                  className="grid grid-cols-[120px_160px_1fr_1fr_130px_130px_60px] gap-0 hover:bg-surface-container transition-colors group"
                >
                  {/* Date */}
                  ...
                  <div className="px-4 py-3 border-r border-outline flex items-center">
                    <p className="text-[12px] text-foreground line-clamp-2">
                      {r.notes || <span className="text-foreground-muted">—</span>}
                    </p>
                  </div>
                  ...
                  <div className="px-4 py-3 border-r border-outline flex items-center">
                    <p className="text-[12px] text-foreground line-clamp-2">
                      {r.clientResponse || <span className="text-foreground-muted">—</span>}
                    </p>
                  </div>
                  ...
                  <div className="px-4 py-3 border-r border-outline flex items-center">
                    <IncludedBadge
                      value={r.weeklyIncluded}
                      onChange={() => toggleField(r.id, "weeklyIncluded")}
                    />
                  </div>
                  ...
                  <div className="px-4 py-3 border-r border-outline flex items-center">
                    <IncludedBadge
                      value={r.monthlyIncluded}
                      onChange={() => toggleField(r.id, "monthlyIncluded")}
                    />
                  </div>
